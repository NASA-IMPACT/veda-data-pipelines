...
class CdkStack(core.Stack):
    def __init__(self, scope: core.Construct, construct_id: str, **kwargs) -> None:
        ...
        database_security_group = ec2.SecurityGroup.from_security_group_id(self, "SG", os.environ.get("SECURITY_GROUP_ID"))

        lambda_function_security_group = ec2.SecurityGroup(
            self,
            f"{id}-{collection}-lambda-sg",
            vpc=database_vpc,
        ...
        s3_discovery_lambda.add_to_role_policy(full_bucket_access)

        archive_lambda = aws_lambda.Function(
            self,
            f"{id}-{bucket}-archive-fn",
            code=aws_lambda.Code.from_asset_image(
                directory="../../lambdas/unarchiver",
                file="Dockerfile",
                entrypoint=["/usr/local/bin/python", "-m", "awslambdaric"],
                cmd=["handler.handler"],
            ),
        ...
        cogify_lambda = aws_lambda.Function(
            self,
            f"{id}-{bucket}-cogify-tiff-fn",
            code=aws_lambda.Code.from_asset_image(
                directory="../../lambdas/cogify-tiff",
                file="Dockerfile",
                entrypoint=["/usr/local/bin/python", "-m", "awslambdaric"],
                cmd=["handler.handler"],
            ),
        ...
        s3_start_state = stepfunctions.Pass(
            self,
            "Start State"
        )
        ...
        cogify_task = tasks.LambdaInvoke(
            self,
            "Cogify Tff",
            lambda_function=cogify_lambda
        )
        ...
        archive_task = tasks.LambdaInvoke(
            self,
            "Unarchiver Task",
            lambda_function=archive_lambda,
            input_path="$.Payload",
        )
        ...
        map_stac_items.iterator(cogify_task.next(s3_generate_stac_item_task.next(s3_db_write_task)))
        """
        map_stac_items.iterator(cogify_task.next(s3_generate_stac_item_task))
        """

        s3_wflow_definition = s3_start_state.next(s3_discover_task).next(archive_task).next(map_stac_items)
